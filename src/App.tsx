...
const App = () => {
  ...
  useEffect(() => {
    ...
    const getDataPeminjam = async () => {
      const { data } = await supabase.from("peminjam").select()
      console.log(data)
      ...
      <div className="flex-1 h-screen overflow-y-auto">
        ...
        <div className="p-5">
          <div className="flex flex-col">
            <span className="text-xs text-zinc-600 w-fit p-2">home{location.pathname}</span>
              ...
              {datasPeminjam.map((item) => {
                return (
                  <div key={item.id} className="p-5 mt-5 flex justify-between items-center border border-zinc-200 bg-white rounded-lg">
                        <div className="flex flex-col">
                      <h1 className="text-zinc-600 font-medium text-sm">{item.nama} meminjam buku <span className="text-blue-500 underline">{item.judul_buku}</span></h1>
                      <span className="text-xs text-zinc-500">{item.created_at.slice(11, 16)}</span>
                        </div>
                        {/* bagde status */}
                        <div className="p-1.5 rounded-md text-xs bg-blue-500/20 w-fit">
